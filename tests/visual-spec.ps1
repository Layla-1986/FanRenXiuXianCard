...
Assert-True (-not $html.Contains('id="agCredits"')) 'Available AI Credits must be removed from the card'
$antigravityTitle = ([char]0x53CD) + ([char]0x91CD) + ([char]0x529B) + ([char]0x5929) + ([char]0x8861)
Assert-True (-not $html.Contains($antigravityTitle)) 'Redundant Antigravity page title must be removed'
...
Assert-True ($html.Contains('id="agGeminiWeeklyReset"')) 'Missing Gemini weekly reset target'
...
Assert-True ($css.Contains('top: 82px')) 'Model ledgers should sit below the facial eye line'
...
Assert-True ($css.Contains('right: 22px')) 'Ledger body should extend toward the right edge'
...
Assert-True ($collector.Contains("'Model Credits'")) 'Antigravity collector must recognize the current Models & Usage quota heading'
Assert-True ($collector.Contains("'Models & Usage'")) 'Antigravity collector must recognize the current Models & Usage page title'
...
Assert-True ($collector.Contains('^Settings - Models$')) 'Antigravity collector may use the exact Settings - Models window title as a bounded fallback'
