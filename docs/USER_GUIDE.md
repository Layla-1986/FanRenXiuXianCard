...
卡片的重置时间按 ChatGPT 使用量面板的 UTC+9 显示，比 Windows 北京时间晚一小时；额度接口的原始时间戳不会改动。
...
这通常表示完整账户接口暂时不可用，应用已回退到本地会话数据。Codex 额度和余额仍会显示能够确认的值；本地会话不包含重置卡数量。请从桌面快捷方式以已登录 Codex CLI 的 Windows 用户启动应用。重新登录或重启后，应用会自动再次尝试只读同步。
